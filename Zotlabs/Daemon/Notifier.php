				...
				if(self::$channel) {
					...
					switch($cmd) {
						case 'permissions_create':
							self::activitypub_permissions_create($perm_update);
							break;
						case 'permissions_accept':
							self::activitypub_permissions_accept($perm_update);
							break;
						default:
							break;
					}
...
//			self::$encoded_item = encode_item($target_item);
	...
	static public function activitypub_process(&$arr) {
		...
		if($prv_recips) {
			...
			foreach($r as $contact) {
				...
				if($single) {
					$qi = self::activitypub_queue_message($jmsg,$arr['channel'],$contact,$target_item['mid']);
					if($qi)
						$arr['queued'][] = $qi;
				}
				continue;
	...
	static function activitypub_queue_message($msg,$sender,$recip,$message_id = '') {
	...
	static function activitypub_permissions_create(&$x) {
		...
		if($single && $h) {
			$qi = self::activitypub_queue_message($jmsg,$x['sender'],$h[0]);
			if($qi)
				$x['deliveries'] = $qi;
		}
	...
	static function activitypub_permissions_accept(&$x) {
