...
class Directory extends DAV\Node implements DAV\ICollection, DAV\IQuota, DAV\IMoveTarget
{
     ...
     * @throw "\Sabre\DAV\Exception\NotFound"
     */
    public function CollectionData($file, &$auth)
    {
        $ret = [];
     ...
     */
    public function ChannelList(&$auth)
    {
        $ret = [];
     ...
     * @param BasicAuth &$auth
     * @param bool $test (optional) enable test mode
     * @return File|Directory|bool|null
     * @throw "\Sabre\DAV\Exception\Forbidden"
     */
    public function FileData($file, &$auth, $test = false)
    {
