	...
	function getFileMimeType(result, address) {
		...
		switch(result.filetype) {
			...
			case "video/ogg":
    			...
    			return `<li class="border rounded my-1 p-2" ><h4  data-bs-dismiss="modal" >${result.filename}<i class="float-end">{{$clicktitle}}</i></h4><video id = "${sendmp4_id}" preload="metadata" src="/cloud/${address}/${result.display_path}" style="width:100%;" /></li>`;
			case "audio/mpeg":
			case "audio/wav":
			case "audio/ogg":
