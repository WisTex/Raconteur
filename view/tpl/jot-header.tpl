		...
		$.post("embedfiles", {},
		    function(data) {
				...
				if (data.success) {
					...
					let content = `<button class="btn" type="button" data-bs-toggle="collapse" data-bs-target="#embedDir-0" aria-expanded="false" aria-controls="embedDir-0"><i class="fa fa-camera-retro fa-lg me-1"></i>${results[0].filename}</button>`;
...
if (results[i].is_dir === "1" && results[(i-1)].is_dir === "1") {
  ...
  content += `<button class="btn" type="button" data-bs-toggle="collapse" data-bs-target="#embedDir-${i}" aria-expanded="false" aria-controls="embedDir-${i}">${results[i].filename}</button>`;
  continue;
