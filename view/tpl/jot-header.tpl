...
let postSaveTimer = null;
	...
	function setEmbedFiles1(result) {
		alert(JSON.stringify(result));
		$.post("wall_attach/ray", result)
  			.done(function(data) {
		...
		switch(result.filetype) {
			...
			//case "image/svg+xml": not working
				...
				{{* return `<li onclick ="setEmbedFiles('${send}')" data-bs-dismiss="modal" ><img src="/cloud/${address}/${result.display_path}" class="img-fluid img-thumbnail" ></li>`; *}}
				  return `<li onclick ="setEmbedFiles1(${result})" data-bs-dismiss="modal" ><img src="/cloud/${address}/${result.display_path}" class="img-fluid img-thumbnail" ></li>`;
			case "video/mp4":
			case "video/webm":
			case "video/ogg":
