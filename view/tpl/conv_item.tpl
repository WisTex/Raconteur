			...
			<div class="clearfix wall-item-content-wrapper{{if $item.is_comment}} comment{{/if}}" id="wall-item-content-wrapper-{{$item.id}}">
				...
				<div class="p-2 clearfix wall-item-tools">
					...
					<div class="wall-item-tools-left btn-group" id="wall-item-tools-left-{{$item.id}}">
						...
						{{if $response.count}}
						<div class="btn-group">
							<button type="button" class="btn btn-outline-secondary btn-sm wall-item-like dropdown-toggle"{{if $response.modal}} data-toggle="modal" data-target="#{{$verb}}Modal-{{$item.id}}"{{else}} data-bs-toggle="dropdown"{{/if}} id="wall-item-{{$verb}}-{{$item.id}}">{{$response.button}}</button>
							{{if $response.modal}}
