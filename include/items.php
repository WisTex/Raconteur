...
function collect_recipients($item, &$private_envelope, $include_groups = true)
{
    ...
    if ($item['allow_cid'] || $item['allow_gid'] || $item['deny_cid'] || $item['deny_gid']) {
        ...
        if ((! $item['allow_cid']) && (! $item['allow_gid'])) {
            $r = q(
                "select * from abook where abook_channel = %d and abook_self = 0 and abook_pending = 0 and abook_archived = 0 ",
                intval($item['uid'])
            );
        ...
        $r = q(
            "select abook_xchan, xchan_network from abook left join xchan on abook_xchan = xchan_hash where abook_channel = %d and abook_self = 0 and abook_pending = 0 and abook_archived = 0 ",
            intval($item['uid'])
        );
        if ($r) {
            foreach ($r as $rv) {
        ...
        if (! intval($item['item_private'])) {
            $r = array_merge(
                ThreadListener::fetch_by_target($item['parent_mid']),
                ThreadListener::fetch_by_target(str_replace('/activity/', '/item/', $item['parent_mid']))
            );
            if ($item['mid'] !== $item['parent_mid']) {
                $r = array_merge(
                    $r,
                    ThreadListener::fetch_by_target($item['mid']),
        ...
        $r = q(
            "select author_xchan from item where parent = %d",
            intval($item['parent'])
        );
        if ($r) {
            foreach ($r as $rv) {
...
function is_item_normal($item)
{
    ...
    if (
        intval($item['item_hidden']) || intval($item['item_type']) || intval($item['item_deleted'])
...
//  logger('Comment_policy: ' . $item['comment_policy'], LOGGER_DEBUG);
    ...
    switch ($item['comment_policy']) {
        ...
        case '':
            ...
            // comment permission
            if (local_channel() && their_perms_contains(local_channel(), $item['owner_xchan'], 'post_comments')) {
                    return true;
            }
...
//  logger('add_source_route ' . $iid . ' ' . $hash, LOGGER_DEBUG);
    ...
    $r = q(
        "select route from item where id = %d limit 1",
        intval($iid)
    );
    if ($r) {
        $new_route = (($r[0]['route']) ? $r[0]['route'] . ',' : '') . $hash;
        q(
            "update item set route = '%s' where id = %d",
            dbesc($new_route),
            intval($iid)
        );
    }
...
function post_activity_item($arr, $allow_code = false, $deliver = true)
{
    ...
    if (
        ! ( array_key_exists('allow_cid', $arr) || array_key_exists('allow_gid', $arr)
...
function get_item_elements($x, $allow_code = false)
{
    ...
    if ($arr['sig']) {
        ...
        $r = q(
            "select xchan_pubkey from xchan where xchan_hash = '%s' limit 1",
...
function import_author_activitypub($x)
{
    ...
    $r = q(
        "select xchan_hash, xchan_url, xchan_network, xchan_name, xchan_photo_s from xchan left join hubloc on xchan_hash = hubloc_hash where hubloc_id_url = '%s'",
        dbesc($x['url'])
    );
    if (! $r) {
        $r = q(
            "select xchan_hash, xchan_url, xchan_network, xchan_name, xchan_photo_s from xchan where xchan_hash = '%s' ",
            dbesc($x['url'])
        );
    }
    if ($r) {
    ...
    if ($z) {
        ...
        if (! $r) {
            $r = q(
                "select xchan_hash, xchan_url, xchan_name, xchan_photo_s from xchan where xchan_hash = '%s' ",
                dbesc($x['url'])
            );
        }
        if ($r) {
...
function import_author_unknown($x)
{
    ...
    $r = q(
        "select xchan_hash from xchan where xchan_network = 'unknown' and xchan_url = '%s' limit 1",
        dbesc($x['url'])
    );
    if ($r) {
    ...
    if ($r && $x['photo']) {
        ...
        if ($photos) {
            $r = q(
                "update xchan set xchan_updated = '%s', xchan_photo_date = '%s', xchan_photo_l = '%s', xchan_photo_m = '%s', xchan_photo_s = '%s', xchan_photo_mimetype = '%s' where xchan_hash = '%s' and xchan_network = 'unknown'",
...
function encode_item($item, $mirror = false)
{
    ...
    $r = q(
        "select channel_id from channel where channel_id = %d limit 1",
        intval($item['uid'])
    );
...
function item_store($arr, $allow_exec = false, $deliver = true, $linkid = true)
{
    ...
    if (array_key_exists('layout_mid', $arr) && $arr['layout_mid']) {
        $l = q(
            "select item_type from item where mid = '%s' and uid = %d limit 1",
    ...
    if ($arr['parent_mid'] === $arr['mid']) {
        ...
        $r = q(
            "SELECT item_level FROM item WHERE mid = '%s' AND uid = %d ORDER BY id ASC LIMIT 1",
        ...
        $r = q(
            "SELECT * FROM item WHERE mid = '%s' AND uid = %d ORDER BY id ASC LIMIT 1",
        ...
        if ($r) {
            ...
            if (($r[0]['mid'] === $r[0]['parent_mid']) && (! intval($r[0]['parent']))) {
                q(
                    "update item set parent = id where id = %d",
                    intval($r[0]['id'])
                );
            }
            ...
            if ($r[0]['mid'] != $r[0]['parent_mid']) {
                ...
                $z = q(
                    "SELECT * FROM item WHERE mid = '%s' AND parent_mid = '%s' AND uid = %d
    ...
    $r = q(
        "SELECT id FROM item WHERE mid = '%s' AND uid = %d and revision = %d LIMIT 1",
        dbesc($arr['mid']),
        intval($arr['uid']),
        intval($arr['revision'])
    );
    ...
    $r = q(
        "SELECT * FROM item WHERE mid = '%s' AND uid = %d and revision = %d ORDER BY id ASC ",
    ...
    if (count($r) > 1) {
        ...
        q(
            "DELETE FROM item WHERE mid = '%s' AND uid = %d AND id != %d ",
            $arr['mid'],
            intval($arr['uid']),
            intval($current_post)
        );
...
function item_update_parent_commented($item)
{
    ...
    if ($update_parent) {
        $z = q(
            "select max(created) as commented from item where parent_mid = '%s' and uid = %d and item_delayed = 0 and item_deleted = 0",
...
function tag_deliver($uid, $item_id)
{
    ...
    if ($is_group && intval($item['item_private']) === 2 && intval($item['item_thread_top']) && (! intval($item['item_wall']))) {
        ...
        if (perm_is_allowed($uid, $item['author_xchan'], 'post_wall')) {
            ...
            q(
                "update item set item_blocked = %d where id = %d",
                intval(ITEM_HIDDEN),
                intval($item_id)
            );
        }
    ...
    if ($is_group && intval($item['item_thread_top']) && intval($item['item_wall']) && $item['author_xchan'] !== $item['owner_xchan']) {
        ...
        start_delivery_chain($u, $item, $item_id, 0, true, (($item['edited'] != $item['created']) || $item['item_deleted']));
        q(
            "update item set item_wall = 0 where id = %d",
            intval($item_id)
        );
        return;
    }
    ...
    if (! intval($item['item_thread_top'])) {
        $x = q(
            "select * from item where id = parent and parent = %d and uid = %d limit 1",
            intval($item['parent']),
            intval($uid)
        );
    ...
    if ($terms) {
        foreach ($terms as $term) {
            ...
            $r = q(
                "update item set item_mentionsme = 1 where id = %d",
                intval($item_id)
            );
...
function tgroup_check($uid, $item)
{
    ...
    if ($item['mid'] !== $item['parent_mid']) {
        $r = q(
            "select id from item where mid = '%s' and uid = %d and item_uplink = 1 limit 1",
            dbesc($item['parent_mid']),
            intval($uid)
        );
        if ($r) {
    ...
    if ($is_group) {
        ...
        if ($item['mid'] === $item['parent_mid'] && (! intval($item['item_wall'])) && (strpos($item['tgt_type'], 'Collection') !== false) && $item['target']) {
            ...
            if (
                (is_string($item['target']) && strpos($item['target'], z_root()) !== false)
...
function start_delivery_chain($channel, $item, $item_id, $parent, $group = false, $edit = false)
{
    ...
    if ($sourced) {
        $r = q(
            "select * from source where src_channel_id = %d and ( src_xchan = '%s' or src_xchan = '*' ) limit 1",
        ...
        if ($rewrite_author) {
            ...
            $r = q(
                "update item set author_xchan = '%s' where id = %d",
                dbesc($item['author_xchan']),
                intval($item_id)
            );
        }
    ...
    if ($group && (! $parent)) {
        ...
        q(
            "update item set item_hidden = 1 where id = %d",
            intval($item_id)
        );
    ...
    if ($parent) {
        $r = q(
            "update item set source_xchan = '%s' where id = %d",
        ...
        if (! $edit) {
            $r = q(
                "update item set source_xchan = owner_xchan where id = %d",
                intval($item_id)
            );
        }
    }
    ...
    $r = q(
        "update item set item_uplink = %d, item_nocomment = %d, item_flags = %d, owner_xchan = '%s', replyto = '%s', allow_cid = '%s', allow_gid = '%s',
    ...
    if ($r) {
        ...
        $r = q(
            "update item set source_xchan = '' where id = %d",
            intval($item_id)
        );
    }
}
...
function check_item_source($uid, $item)
{
    ...
    $x = q(
        "select abook_feed from abook where abook_channel = %d and abook_xchan = '%s' limit 1",
        intval($uid),
        dbesc($xchan)
    );
...
{
    if (
        (isset($ob['allow_cid']) && $obj['allow_cid'] != '')
...
{
    $r = q(
        "update item set item_retained = 1 where id = %d",
        intval($id)
    );
}
...
function drop_item($id, $interactive = true, $stage = DROPITEM_NORMAL, $force = false)
{
    ...
    if ($ok_to_delete) {
        ...
        $items = q(
            "select * from item where parent = %d and uid = %d",
            intval($item['id']),
            intval($item['uid'])
        );
        if ($items) {
...
function delete_item_lowlevel($item, $stage = DROPITEM_NORMAL, $force = false)
{
    ...
    switch ($stage) {
        case DROPITEM_PHASE2:
            $r = q(
                "UPDATE item SET item_pending_remove = 1, body = '', title = '',
        ...
        case DROPITEM_PHASE1:
            $r = q(
                "UPDATE item set item_deleted = 1, changed = '%s', edited = '%s' where id = %d",
                dbesc(datetime_convert()),
                dbesc(datetime_convert()),
                intval($item['id'])
            );
    ...
    q(
        "delete from likes where iid = %d and channel_id = %d",
        intval($item['id']),
        intval($item['uid'])
    );
    ...
    if ($c) {
        q(
            "delete from dreport where dreport_xchan = '%s' and dreport_mid = '%s'",
...
function fetch_post_tags($items, $link = false)
{
    ...
    if (strlen($tag_finder_str)) {
        $tags = q(
            "select * from term where oid in ( %s ) and otype = %d",
            dbesc($tag_finder_str),
            intval(TERM_OBJ_POST)
        );
        $imeta = q(
            "select * from iconfig where iid in ( %s )",
            dbesc($tag_finder_str)
        );
    }
...
function items_fetch($arr, $channel = null, $observer_hash = null, $client_mode = CLIENT_MODE_NORMAL, $module = 'stream')
{
    ...
    if (isset($arr['gid']) && $arr['gid'] && $uid) {
        ...
        $r = q(
            "SELECT abook.*, xchan.* from abook left join xchan on abook_xchan = xchan_hash where abook_id = %d and abook_channel = %d and abook_blocked = 0 limit 1",
            intval($arr['cid']),
    ...
    if (isset($arr['conv']) && $arr['conv'] && $channel) {
        $sql_extra .= sprintf(
            " AND parent IN (SELECT distinct parent from item where ( author_xchan like '%s' or item_mentionsme = 1 )) ",
...
{
    $r = q(
        "select id from item where mid = '%s' and uid = %d and allow_cid like '%s'",
        dbesc($mid),
        intval($uid),
        dbesc('<' . $xchan_hash . '>')
    );
    if (! $r) {
        $r = q(
            "update item set allow_cid = concat(allow_cid,'%s') where mid = '%s' and uid = %d",
            dbesc('<' . $xchan_hash . '>'),
            dbesc($mid),
            intval($uid)
        );
...
{
    $r = q(
        "select allow_cid from item where mid = '%s' and uid = %d and allow_cid like '%s'",
        dbesc($mid),
        intval($uid),
        dbesc('<' . $xchan_hash . '>')
    );
    if ($r) {
        $x = q(
            "update item set allow_cid = '%s' where mid = '%s' and uid = %d",
...
function fix_attached_permissions($uid, $body, $str_contact_allow, $str_group_allow, $str_contact_deny, $str_group_deny, $token = EMPTY_STR)
{
    ...
    foreach ($files as $file) {
        $attach_q = q(
            "select id, hash, flags, is_photo, allow_cid, allow_gid, deny_cid, deny_gid from attach where hash = '%s' and uid = %d",
            dbesc($file),
            intval($uid)
        );
        ...
        q(
            "update attach SET allow_cid = '%s', allow_gid = '%s', deny_cid = '%s', deny_gid = '%s', flags = 0
        ...
        if ($attach['is_photo']) {
            $r = q(
                "UPDATE photo SET allow_cid = '%s', allow_gid = '%s', deny_cid = '%s', deny_gid = '%s'
            ...
            $r = q(
                "UPDATE item SET allow_cid = '%s', allow_gid = '%s', deny_cid = '%s', deny_gid = '%s', item_private = %d
...
function copy_of_pubitem($channel, $mid)
{
    ...
    $r = q(
        "select * from item where parent_mid = ( select parent_mid from item where mid = '%s' and uid = %d ) order by id ",
        dbesc($mid),
    ...
    if (! $r) {
        $r = q(
            "select * from item where parent_mid = ( select distinct (parent_mid) from item where mid = '%s' and item_wall = 1 and item_private = 0 ) order by id ",
            dbesc($mid),
            intval($syschan['channel_id'])
        );
    }
