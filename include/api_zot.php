...
function api_channel_list($type)
{
    ...
    $r = q(
        "select channel_address from channel where channel_account_id = %d",
...
function api_file_meta($type)
{
    ...
    $r = q(
        "select * from attach where uid = %d and hash = '%s' limit 1",
        intval(api_user()),
...
function api_file_data($type)
{
    ...
    $r = q(
        "select * from attach where uid = %d and hash like '%s' limit 1",
        intval(api_user()),
        dbesc($_REQUEST['file_id'] . '%')
    );
    if ($r) {
...
function api_photo_detail($type)
{
    ...
    $r = q(
        "select * from photo where uid = %d and resource_id = '%s' and imgscale = %d limit 1",
    ...
    if ($r) {
        ...
        if ($i) {
            $ii = q(
                "select * from item where parent = %d order by id",
                intval($i[0]['id'])
            );
            if ($ii) {
...
function api_group_members($type)
{
    ...
    if ($_REQUEST['group_id']) {
        $r = q(
            "select * from pgrp where uid = %d and id = %d limit 1",
            intval(api_user()),
            intval($_REQUEST['group_id'])
        );
    } elseif ($_REQUEST['group_name']) {
        $r = q(
            "select * from pgrp where uid = %d and gname = '%s' limit 1",
            intval(api_user()),
    ...
    if ($r) {
        $x = q(
            "select * from pgrp_member left join abook on abook_xchan = xchan and abook_channel = pgrp_member.uid left join xchan on pgrp_member.xchan = xchan.xchan_hash 
				where gid = %d",
            intval($r[0]['id'])
        );
        json_return_and_die($x);
...
function api_zot_abook_xchan($type)
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $r = q(
            "select * from abook left join xchan on abook_xchan = xchan_hash where abook_channel = %d $sql_extra ",
            intval(api_user())
        );
...
function api_zot_abconfig($type)
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $r = q(
            "select abconfig.* from abconfig left join abook on abook_xchan = abconfig.xchan and abook_channel = abconfig.chan where abconfig.chan = %d $sql_extra ",
            intval(api_user())
        );
...
function api_zot_perm_allowed($type)
{
    ...
    if (array_key_exists('abook_id', $_REQUEST) && intval($_REQUEST['abook_id'])) {
        $r = q(
            "select abook_xchan as hash from abook where abook_id = %d and abook_channel = %d limit 1",
