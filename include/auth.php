...
function account_verify_password($login, $pass)
{
    ...
    if (($addon_auth['authenticated']) && is_array($addon_auth['user_record']) && (! empty($addon_auth['user_record']))) {
        ...
        if (
            ($account['account_flags'] == ACCOUNT_OK)
...
if (
    ...
    if (x($_SESSION, 'visitor_id') && (! x($_SESSION, 'uid'))) {
        ...
        if ($admins && is_array($admins) && in_array($_SESSION['visitor_id'], $admins)) {
            $x = q(
                "select * from account where account_email = '%s' and account_email != '' and ( account_flags & %d ) > 0 limit 1",
        ...
        if (array_key_exists('atoken', $_SESSION)) {
            $y = q(
                "select * from atoken where atoken_id = %d limit 1",
...
function match_openid($authid)
{
    ...
    $r = q(
        "SELECT uid FROM pconfig WHERE cat = 'system' AND k = 'openid' AND v = '%s' LIMIT 1",
        dbesc($authid)
    );
    if ($r) {
        return $r[0]['uid'];
